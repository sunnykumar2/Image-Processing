...
inputImage = cv2.imread("line.jpeg",0)
...
def PerformSobel(image):
    kernelY = np.asarray([[-1,2,-1],[-1,2,-1],[-1,2,-1]])
    gradientY = Convolution(image, kernelY)
    cv2.imshow("gradient_y",gradientY)
    
    
    kernelX = np.asarray([[-1,-1,-1],[2,2,2],[-1,-1,-1]])
    gradientX = Convolution(image, kernelX)
    cv2.imshow("gradient_x", gradientX)    
    
    
    kernel45 = np.asarray([[-1,-1,2],[-1,2,-1],[2,-1,-1]])
    gradient45 = Convolution(image, kernel45)
    cv2.imshow("gradient-45", gradient45)    
    
    kernel_45 = np.asarray([[2, -1, -1], [-1, 2, -1], [-1, -1, 2]])
    gradient_45 = Convolution(image, kernel_45)
    cv2.imshow("gradient+45", gradient_45)
